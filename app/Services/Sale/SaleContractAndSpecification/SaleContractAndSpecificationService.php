...
namespace App\Services\Sale\SaleContractAndSpecification;
use App\Models\Sale\SaleContractAndSpecification;

class SaleContractAndSpecificationService
{
     public static function index($requestAll) {
        try {
            ...
            $offset = $limit * ($page-1);
            $model = SaleContractAndSpecification::where(['deleted_at' => null]);
               // ->with([])
            ...
            return [
                'data' => $data,
                'data_total' => [
                    'summ' => SaleContractAndSpecification::where($filter)
                        ->where(['deleted_at' => null])
                        ->sum('summ'),
                    'summ_nds' => SaleContractAndSpecification::where($filter)
                        ->where(['deleted_at' => null])
                        ->sum('summ_nds'),
    ...
    public static function create($data){
        try {
            return SaleContractAndSpecification::create($data);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function card($id){ 
        return SaleContractAndSpecification::where(['id' => $id])
            //->with([])
            ->first();    
    }
    public static function update($id, $data){ 
        try {
            SaleContractAndSpecification::where('id', $id)->first()->update($data);
            return SaleContractAndSpecification::where('id', $id)
                //->with([])
                ->first();
    ...
    public static function field($id, $field){ 
        try {
            $result = SaleContractAndSpecification::where('id', $id)->select($field)->first();
            if(!$result ) return;
            return $result[$field];
        } catch (Exception $e) {
            return $e->getMessage();
