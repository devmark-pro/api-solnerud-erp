...
namespace App\Services\Sale\SaleAccountSupplier;
use App\Models\Sale\SaleAccountSupplier;

class SaleAccountSupplierService
{
     public static function index($requestAll) {
        try {
            ...
            $offset = $limit * ($page-1);
            $model = SaleAccountSupplier::where(['deleted_at' => null]);
               // ->with([])
    ...
    public static function create($data){
        try {
            return SaleAccountSupplier::create($data);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function card($id){ 
        return SaleAccountSupplier::where(['id' => $id])
            //->with([])
            ->first();    
    }
    public static function update($id, $data){ 
        try {
            SaleAccountSupplier::where('id', $id)->first()->update($data);
            return SaleAccountSupplier::where('id', $id)
                //->with([])
                ->first();
    ...
    public static function field($id, $field){ 
        try {
            $result = SaleAccountSupplier::where('id', $id)->select($field)->first();
            if(!$result ) return;
            return $result[$field];
        } catch (Exception $e) {
            return $e->getMessage();
