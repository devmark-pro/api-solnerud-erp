...
class SaleInvoiceService
{
     public static function index($requestAll) {
        try {
            ...
            $offset = $limit * ($page-1);
            $model = SaleInvoice::where(['deleted_at' => null]);
               // ->with([])
    ...
    public static function create($data){
        try {
            return SaleInvoice::create($data);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function card($id){ 
        return SaleInvoice::where(['id' => $id])
            //->with([])
            ->first();    
    }
    public static function update($id, $data){ 
        try {
            SaleInvoice::where('id', $id)->first()->update($data);
            return SaleInvoice::where('id', $id)
                //->with([])
                ->first();
    ...
    public static function delete($id){ 
        try {
            return SaleInvoice::where('id', $id)->first()->update(['deleted_at' => now()]);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function recover($id){ 
        try {
            return SaleInvoice::where('id', $id)->fitst()->update(['deleted_at' => null]);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function field($id, $field){ 
        try {
            $result = SaleInvoice::where('id', $id)->select($field)->first();
            if(!$result ) return;
            return $result[$field];
        } catch (Exception $e) {
            return $e->getMessage();
