...
class ClientWarehouseService
{
     public static function index($requestAll) {
        try {
            ...
            $offset = $limit * ($page-1);
            $model = ClientWarehouse::where(['deleted_at' => null]);
               // ->with([])
    ...
    public static function create($data){
        try {
            return ClientWarehouse::create($data);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function card($id){ 
        return ClientWarehouse::where(['id' => $id])
            //->with([])
            ->first();    
    }
    public static function update($id, $data){ 
        try {
            ClientWarehouse::where('id', $id)->first()->update($data);
            return ClientWarehouse::where('id', $id)
                //->with([])
                ->first();
    ...
    public static function delete($id){ 
        try {
            return ClientWarehouse::where('id', $id)->update(['deleted_at' => now()]);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function recover($id){ 
        try {
            return ClientWarehouse::where('id', $id)->update(['deleted_at' => null]);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function field($id, $field){ 
        try {
            $result = ClientWarehouse::where('id', $id)->select($field)->first();
            if(!$result ) return;
            return $result[$field];
        } catch (Exception $e) {
            return $e->getMessage();
