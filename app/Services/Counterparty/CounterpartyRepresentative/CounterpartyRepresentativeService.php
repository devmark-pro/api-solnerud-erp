...
namespace App\Services\Counterparty\CounterpartyRepresentative;
use App\Models\Counterparty\CounterpartyRepresentative;

class CounterpartyRepresentativeService
{
     public static function index($requestAll) {
        try {
            ...
            $offset = $limit * ($page-1);
            $model = CounterpartyRepresentative::where(['deleted_at' => null])
               ->with(['representativePosition']);
    ...
    public static function create($data){
        try {
            return CounterpartyRepresentative::create($data);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function card($id){ 
        return CounterpartyRepresentative::where(['id' => $id])
            //->with([])
            ->first();    
    }
    public static function update($id, $data){ 
        try {
            CounterpartyRepresentative::where('id', $id)->update($data);
            return CounterpartyRepresentative::where('id', $id)
                // ->with([])
                ->first();
