...
class CounterpartyWarehouseService
{
     public static function index($requestAll) {
        try {
            ...
            $offset = $limit * ($page-1);
            $model = CounterpartyWarehouse::where(['deleted_at' => null]);
               // ->with([])
    ...
    public static function create($data){
        try {
            return CounterpartyWarehouse::create($data);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function card($id){ 
        return CounterpartyWarehouse::where(['id' => $id])
            //->with([])
            ->first();    
    }
    public static function update($id, $data){ 
        try {
            CounterpartyWarehouse::where('id', $id)->first()->update($data);
            return CounterpartyWarehouse::where('id', $id)
                //->with([])
                ->first();
    ...
    public static function delete($id){ 
        try {
            return CounterpartyWarehouse::where('id', $id)->update(['deleted_at' => now()]);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function recover($id){ 
        try {
            return CounterpartyWarehouse::where('id', $id)->update(['deleted_at' => null]);
        } catch (Exception $e) {
            return $e->getMessage();
        }
    }
    public static function field($id, $field){ 
        try {
            $result = CounterpartyWarehouse::where('id', $id)->select($field)->first();
            if(!$result ) return;
            return $result[$field];
        } catch (Exception $e) {
            return $e->getMessage();
