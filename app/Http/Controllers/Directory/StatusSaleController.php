...
public function index(Request $request)
    ...
    public function create(Request $request)
    {
        try {
            ...
            return StatusSaleService::create($updateData);
        } catch (Exception $e){
            return $e->getMessage();
        }
    ...
    public function update(Request $request)
    {
        try {
            ...
            $validator = Validator::make($requestData, [
                'id'=>'required',
                'data'=>'required',
                // 'data.name'=>'required|unique:directory_status_sales',
            ]);
            if($validator->fails()){
                $error = $validator->errors()->toArray();
                return response()->json(['message'=>$error])->setStatusCode(417); 
            ...
            $validator = Validator::make($requestData['data'], [
                'name'=>'required|unique:directory_status_sales,name,'.$requestData['id'],
            ]);
            if($validator->fails()){
                $error = $validator->errors()->toArray();
                return response()->json(['message'=>$error])->setStatusCode(417); 
            ...
            $result = StatusSaleService::update($id, $data);
